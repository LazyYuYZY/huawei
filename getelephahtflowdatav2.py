...
# ------------------- 参数设置部分 -------------------
# 定义总流量数量
num_total_flows = (120*1000)//176+10
# 定义小流所占比例
small_ratio = 0.9
# 定义大流所占比例
large_ratio = 0.1
...
def generate_packets(flow_id, total_bytes, packet_bytes, ports):
    ...
    for i, port in enumerate(ports):
        ...
        # 拆分子流方便记录
        size_slice= size // slice_num
        size_last = size % slice_num
        # 记录大小为1的子流数量
        if (size_slice == 1 and size_last == 0) or (size_slice == 0 and size_last != 0):
            one_subflows.append((flow_id, port))
        # 记录大小为2的子流数量
        if (size_slice == 2 and size_last == 0) or (size_slice == 1 and size_last != 0):
        ...
        # 注意：这里的 size_last 可能为 0，所以需要判断
        if size_last > 0:
            packet_seq[-1].append((flow_id, port, size_last))
...
with open("flow_port_mixedv2.txt", "w") as f:
