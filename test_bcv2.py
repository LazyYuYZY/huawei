...
parameters = {
    "hash_table_size": 2000,  # 哈希表大小
    "hash_table_bit_width": 8,  # 哈希表位宽
    'k1': 2, 'k2': 2, 'k3': 2, 'd4': 3, 'm1': 160000, 'm2': 60000, 'm3': 40000, 'w4': 10000 
}
...
def classify_subflows_by_size_cmsketch(overflow_flows):
    """
    以下针对真实插入sketch的子流进行分类。
    分类子流为三类集合：大小等于1、等于2、大于2。
    返回：eq_1_set, eq_2_set, gt_2_set
    """
    # 使用 defaultdict 初始化一个字典，用于记录每个子流的总大小
...
def load_flow_data(file_path):
    ...
    true_flow_ids = set()
    # 初始化一个集合，用于存储 Sketch 中真实插入的 flow_id
    true_flow_ids_sketch = set()

    # 初始化一个 FixedHashTable 对象，用于缩放子流的大小
    hash_table = FixedHashTable(size=parameters['hash_table_size'], bit_width=parameters['hash_table_bit_width'])
    ...
    with open(file_path, "r") as f:
        ...
        for line in f:
            ...
            # 将 flow_key 和对应的大小插入到 hashtable 中
            hash_table.insert(flow_key, size)
            if size==64:
                counter_64+=1
        print("counter_64:",counter_64)
        
        # 获取溢出流量的 flow_id 列表
        overflow_flows = hash_table.get_overflowed()
        # print(overflow_flows)
        # 打印溢出流量的数量
        print(f"Overflow flows count: {len(overflow_flows)}")
        # 获取hashtable冲突上报的 flow_id 列表
        collision_flows = hash_table.get_conflicted()
        # 打印冲突流量的数量
        print(f"Collision flows count: {len(collision_flows)}")

        # 定义溢出流量列表
        overflow_flows_seq = []
        seq_len = 5
        # 将溢出流量按 flow_key 进行序列化
        for pair in overflow_flows:
            # print(f"Overflow flow: {pair}")
            flow_key, overflow_times = pair
            # if overflow_times == 1:
            #     print(f"Warning: flow {flow_key} has overflow_times = 1, which is not expected.")
            if overflow_times <= seq_len: # 当size为0时，不生成数据包
                # 若该端口的数据包数量小于等于 seq_len，则生成对应数量的数据包，每个数据包大小为 slice_num
                overflow_flows_seq.append([(flow_key, 1)] * overflow_times)
            else:
                # 若该端口的数据包数量大于 seq_len，先生成 seq_len 个大小为 slice_num 的数据包
                first_part = [(flow_key, 1)] * seq_len
                # 再生成一个大小为剩余数据包数的数据包
                remainder = [(flow_key, overflow_times - seq_len)]
                overflow_flows_seq.append(first_part + remainder)
        # 将溢出流量序列化后的列表合并为一个大列表
        overflow_flows_seq = [item for sublist in overflow_flows_seq for item in sublist]

            
        print(f"Overflow flows sequence length: {len(overflow_flows_seq)}")
        counter_observer = 0
        # 遍历溢出流量列表，将其插入到 Sketch 中
        for pair in overflow_flows_seq:
            # print(f"Processing overflow flow: {pair}")
            flow_key, overflow_times = pair
            # # 观测者计数器，如果小于1000，则打印插入的 flow_key 和 overflow_times
            # if counter_observer< 1000:
            #     print(f"Inserting flow_key: {flow_key}, overflow_times: {overflow_times}")
            #     counter_observer += 1
            # 将真实插入sketch的flow_key记录
            true_flow_ids_sketch.add(flow_key)
            # 将 overflow 总量插入 sketch
            sketch.insert(flow_key, overflow_times)
    print(f"Max Count in Sketch: {sketch.query_cm_max_count()}")
    return sketch, true_flow_ids, true_flow_ids_sketch, overflow_flows, collision_flows
...
def main():
    ...
    sketch, true_flow_ids, true_flow_ids_sketch, overflow_flows, collision_flows = load_flow_data("flow_port_mixedv2.txt")

    # 分类子流，将真实插入 Sketch 的子流按大小分为三类集合
    s1, s2, s3, true_flow_size = classify_subflows_by_size_cmsketch(overflow_flows)
    print(f"Size 1: {len(s1)}")
    print(f"Size 2: {len(s2)}")
    print(f"Size > 2: {len(s3)}")
    ...
    dataplane_report, report_stats,counter_report = query_and_report(sketch, true_flow_ids_sketch)

    # 评估误报率，计算并打印误报和漏报相关指标
    fp_stats = estimate_fp_fn_rates(sketch, true_flow_ids_sketch)
    ...
    # 计算无误差的流的数量
    no_error_count += (len(true_flow_ids)-len(true_flow_ids_sketch))
    print(f"no error count: {no_error_count}")
    ...
    results = {
        ...
        "total_inserted": len(true_flow_ids_sketch),  # 插入的流的数量
